#include "ZmienneUniformPrzesuniecia.h"
...
void  ZmienneUniform::wyswietl()
{
	...
	int milisekudny = glutGet(GLUT_ELAPSED_TIME);
	GLfloat move = sinf(milisekudny / 1000.0 * (2 * 3.14) / 5);

	glm::vec3 translate = glm::vec3(1.2f, 1.2f, 0.0f);
	glm::mat4 modelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(move, move, 0.0f));
	
	
	
	glUniformMatrix4fv(zmianaShader, 1, GL_FALSE, glm::value_ptr(modelMatrix));
...
{
	glm::vec3 wierzcholek1 = glm::vec3(-0.2f, -0.2f, 0.0f);
	glm::vec3 wierzcholek2 = glm::vec3(0.2f, -0.2f, 0.0f);
	glm::vec3 wierzcholek3 = glm::vec3(0.0f, 0.2f, 0.0f);
...
void ZmienneUniform::stworzenieProgramu()
{
	...
	static const GLchar * vertexShader = {
		"	#version 330 core \n																	\
			...
			void main()																				\
			{																						\
				gl_Position = zmiana * vec4(polozenie, 1.0f);	\
			}"
	};
